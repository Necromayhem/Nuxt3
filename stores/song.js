import { defineStore } from 'pinia'
import artist from '@/artist.json' // Убедитесь, что путь к JSON-у корректный

export const useSongStore = defineStore('song', {
	state: () => ({
		isPlaying: false,
		audio: null,
		currentArtist: null,
		currentTrack: null,
		trackTime: null,
		currentVolume: 80,
		isLyrics: false,
		lyricsPosition: '0:00',
	}),
	actions: {
		loadSong(artist, track) {
			this.currentArtist = artist
			this.currentTrack = track

			// Проверяем доступность объекта "Audio", чтобы избежать ошибок рендеринга на сервере
			if (process.client) {
				if (this.audio && this.audio.src) {
					this.audio.pause()
					this.isPlaying = false
					this.audio.src = ''
				}

				this.audio = new Audio()
				this.audio.src = track.path

				setTimeout(() => {
					this.isPlaying = true
					this.audio.play()
				}, 200)
			}
		},

		playOrPauseSong() {
			// Подстраховка для выполнения только в клиентской среде
			if (process.client && this.audio) {
				if (this.audio.paused) {
					this.isPlaying = true
					this.audio.play()
				} else {
					this.isPlaying = false
					this.audio.pause()
				}
			}
		},

		playOrPauseThisSong(artist, track) {
			if (
				!this.audio ||
				!this.audio.src ||
				this.currentTrack?.id !== track.id
			) {
				this.loadSong(artist, track)
				return
			}

			this.playOrPauseSong()
		},

		prevSong(currentTrack) {
			let track = artist.tracks[currentTrack.id - 2]
			this.loadSong(artist, track)
		},

		nextSong(currentTrack) {
			if (currentTrack.id === artist.tracks.length) {
				let track = artist.tracks[0]
				this.loadSong(artist, track)
			} else {
				let track = artist.tracks[currentTrack.id]
				this.loadSong(artist, track)
			}
		},

		playFromFirst() {
			this.resetState()
			let track = artist.tracks[0]
			this.loadSong(artist, track)
		},

		resetState() {
			this.isPlaying = false
			this.audio = null
			this.currentArtist = null
			this.currentTrack = null
		},
	},
	// Если нужно сохранять состояние хранилища (например, между перезагрузками страницы)
	persist: true, // Используйте специальный модуль pinia-plugin-persistedstate
})
